...
public class ModMenu extends GuiScreen {
  ...
  public ModMenu(GuiScreen guiscreen) {
	...
	* Initialise settings
	*/
	private ModMenu(GuiScreen parent, String title, boolean world, boolean mult) {
		super();
		parentScreen 		= parent;
		worldMode 			= world;
		screenTitle			= title;
	...
	private void addModOptionButton(ModOption op, int id, int pos) {
  	...
  	yPos = 41 + ((pos>>1)*24);
		if(op instanceof ModSliderOption) {
			...
			controlList.add(new TextField(id, this, fontRenderer, xPos, yPos, (ModTextOption) op, !worldMode));
		} else if(op instanceof ModKeyOption) {
			KeyBindingField btn = new KeyBindingField(id, this, fontRenderer, xPos, yPos, (ModKeyOption) op, !worldMode);	
			btn.setWide(isWide);
			controlList.add(btn);
		} else if(!isWide) {
			// 150, 20 is the width/ height for a small button
			controlList.add(new Button(id, xPos, yPos, 150, 20, op, worldMode));
		} else {
			controlList.add(new Button(id, xPos, yPos, op, worldMode));
		}
	}
  ...
  protected void mouseMovedOrUp(int i, int j, int k) {
		...
		if(!(curButton instanceof TextInputField) && curButton != null && k == 0) {
	...
	*/
	private String optionPressed(ModMultiOption op) {
		...
		return op.getDisplayString(worldMode);
	}
